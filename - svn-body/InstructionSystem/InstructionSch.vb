...
#Region "PUBLIC MEMBER"
    ...
    Public m_iMaxRowCount As Integer = ModConstant.INSTRUCTION_DATA_SIZE_PER_PAGE
    Private Const SCREEN_NAME As String = "INSTRUCTION_SCHEDULE"
...
#End Region
    ...
    Private Sub GetDataByLine(ByVal lineNo As Integer)
        ...
        Try
            ...
            tmpDtIns = taInstruction.GetDataByCurrentInstructionRow(lineNo)
            ...
            Else
                ...
                Else
                    ...
                    DataGridIdata.DataSource = Nothing
                    MessageBox.Show("No Instruction Schedule")
                End If
                'tmpDtIns = taInstruction.GetDataByPreviousBtn(CInt(m_iMaxRowCount / 2), prodDate, prodTime, seqNo, subSeq, m_iLineNo)
            End If
...
#Region "RELOAD BUTTON"
    ...
    Private Sub btnF9_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnF9.Click
        ...
        If DataGridIdata.RowCount > 0 Then
            Dim taInstruction As DataSet1TableAdapters.T_Instruction_DATTableAdapter = Me.m_taInst
            Dim dt As DataSet1.T_Instruction_DATDataTable = Me.m_dtInst
            ...
            prodDate = DataGridIdata.Item(11, 0).Value
            prodTime = DataGridIdata.Item(12, 0).Value
            seqNo = DataGridIdata.Item(1, 0).Value
            subSeq = DataGridIdata.Item(2, 0).Value
            Try
                ...
                If dt.Rows.Count > 0 Then
                    Me.DataGridIdata.DataSource = dt
                End If
            Catch ex As Exception
...
#Region "NEXT BUTTON"
    ...
    Private Sub btnF8_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnF8.Click
        ...
        Try
            If DataGridIdata.Rows.Count = m_iMaxRowCount Then
                prodDate = DataGridIdata.Item(11, m_iMaxRowCount - 1).Value
                prodTime = DataGridIdata.Item(12, m_iMaxRowCount - 1).Value
                seqNo = DataGridIdata.Item(1, m_iMaxRowCount - 1).Value
                subSeq = DataGridIdata.Item(2, m_iMaxRowCount - 1).Value


                logger.Log(Common.Logger.MessageType.NormalLog, Me.GetType.Name, "btnF8_Click", "Before Getdata" & Now.Second & Now.Millisecond, "")
                dt = GetData(m_iLineNo, prodDate, prodTime, seqNo, subSeq)

                logger.Log(Common.Logger.MessageType.NormalLog, Me.GetType.Name, "btnF8_Click", "After" & Now.Second & Now.Millisecond, "")
                If dt.Rows.Count > 0 Then
                    Me.DataGridIdata.DataSource = dt
                End If

                logger.Log(Common.Logger.MessageType.NormalLog, Me.GetType.Name, "btnF8_Click", "After Datagrid" & Now, "")
            End If
...
#End Region
    ...
    Private Sub InstructionSch_FormClosed(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        logger.Log(Common.Logger.MessageType.NormalLog, Me.GetType.Name, "btnF12_Click", "Click Close Instruction Screen", "")

        Dim colObj As clsColumnSetup = New clsColumnSetup()
        'colObj.writeInsColum(DataGridIdata, SCREEN_NAME)
        colObj.setColumnIni(DataGridIdata, SCREEN_NAME)
        Me.Dispose()
        Main_Menu.Show()
    End Sub

    Private Sub InstructionSch_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        Select Case e.KeyData
            ...
            Case Keys.Up
                If Me.DataGridIdata.CurrentRow.Index = 0 Then
                    'btnF7_Click(sender, e)
                    If DataGridIdata.RowCount > 0 Then
                        Dim dt As DataSet1.T_Instruction_DATDataTable = Me.m_dtInst
                        Dim taInstruction As DataSet1TableAdapters.T_Instruction_DATTableAdapter = Me.m_taInst
                        ...
                        Catch ex As Exception
                            ...
                            MessageBox.Show(ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
                            logger.Log(Common.Logger.MessageType.ErrorLog, Me.GetType.Name, "InstructionSch_KeyDown", ex.Message, "")
                        End Try
                    End If
                End If

            Case Keys.Down
                If Me.DataGridIdata.CurrentRow.Index = Me.DataGridIdata.RowCount - 1 Then
                    ...
                    Try
                        ...
                        Me.DataGridIdata.Rows(Me.DataGridIdata.RowCount - 1).Cells(0).Selected = True
    ...
    Private Sub InstructionSch_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load

        Try
            logger.Log(Common.Logger.MessageType.NormalLog, Me.GetType.Name, "InstructionSch_Load", "Load Instruction Screen", "")

            'TODO: This line of code loads data into the 'DataSet1.T_Instruction_DAT' table. You can move, or remove it, as needed.
            'Me.T_Instruction_DATTableAdapter.Fill(Me.DataSet1.T_Instruction_DAT)
            ...
            Dim colObj As clsColumnSetup = New clsColumnSetup()
            setDatagridColWidthIni(colObj.getColumnIni(DataGridIdata.Columns.Count, SCREEN_NAME))

            calCol()

        Catch ex As Exception
            logger.Log(Common.Logger.MessageType.ErrorLog, Me.GetType.Name, "InstructionSch_Load", ex.Message, "")
    ...
    Private Sub setDatagridColWidthIni(ByVal arr As String())
        ...
        For i = 0 To array.Length - 1
            DataGridIdata.Columns(i).Width = Integer.Parse(array(i))
        Next
    End Sub
    ...
    Private Sub setDatagridColor()
        Try
            ...
            Me.m_dtTempInst = ta.GetDataByCurrentInstructionRow(m_iLineNo)
            ...
            Me.m_dtInst = Me.DataGridIdata.DataSource

            Do While row < Me.DataGridIdata.RowCount
                ...
                If Not IsNothing(drCurrCarry) AndAlso (drCurrCarry.ModelYear = drDatagrid.ModelYear _
                    ...
                    Me.DataGridIdata.Rows(row).DefaultCellStyle.BackColor = Color.Orange
                Else

                    If drDatagrid.IsInsResultNull Then 'No Result

                        If drDatagrid.XchgFlag.Substring(0, 1) = "2" Then
                            Me.DataGridIdata.Rows(row).DefaultCellStyle.BackColor = Color.Yellow
                        End If

                        If drDatagrid.XchgFlag.Substring(0, 1) = "4" Then

                            If drDatagrid.YChassisFlag = "0" Or drDatagrid.YChassisFlag = "" Then
                                Me.DataGridIdata.Rows(row).DefaultCellStyle.BackColor = Color.ForestGreen
                            End If

                            If drDatagrid.YChassisFlag = "1" Or drDatagrid.YChassisFlag = "2" Then
                                Me.DataGridIdata.Rows(row).DefaultCellStyle.BackColor = Color.LimeGreen
                            End If

                            If drDatagrid.YChassisFlag = "3" Then
                                Me.DataGridIdata.Rows(row).DefaultCellStyle.BackColor = Color.LightGreen
                            End If

                        End If

                        If drDatagrid.XchgFlag.Substring(2, 2) = "01" Then
                            Me.DataGridIdata.Rows(row).DefaultCellStyle.BackColor = Color.Aqua
                        End If

                        If drDatagrid.XchgFlag.Substring(2, 2) = "02" Then
                            Me.DataGridIdata.Rows(row).DefaultCellStyle.BackColor = Color.Magenta
                        End If

                    Else 'have Result
                        If drDatagrid.XchgFlag = "0000" Then
                            Me.DataGridIdata.Rows(row).DefaultCellStyle.BackColor = Color.Gray
                        End If
    ...
    Private Sub DataGridIdata_DataSourceChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles DataGridIdata.DataSourceChanged
    ...
    Private Sub InstructionSch_Shown(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Shown
        setDatagridColor()
    End Sub
        ...
        If MessageBox.Show("Do you want to change it?", "Confirmation", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) = Windows.Forms.DialogResult.OK Then
            ...
            Try
                ...
                If DataGridIdata.RowCount > 0 Then
                    ...
                    SelectedIndexFull = DataGridIdata.CurrentRow.Cells(0).Value
                    ...
                    'Dim SelprodDate As String = DataGridIdata.CurrentRow.Cells(11).Value
                    'Dim SelprodTime As String = DataGridIdata.CurrentRow.Cells(12).Value
                    'Dim SelseqNo As String = DataGridIdata.CurrentRow.Cells(1).Value
                    'Dim SelsubSeq As Integer = DataGridIdata.CurrentRow.Cells(2).Value
                    'Dim Selindex As Integer = DataGridIdata.CurrentRow.Cells(0).Value
                    ...
                    Dim dtSelected As DataSet1.T_Instruction_DATDataTable = DataGridIdata.DataSource
                    Dim drSelectedRow As DataSet1.T_Instruction_DATRow = dtSelected.Rows(DataGridIdata.CurrentRow.Index)
                    ...
                    If compareRow > 0 Then
                        ...
                        For Each dr As DataSet1.T_Instruction_DATRow In dt
                            dr.InsResult = dateNow
                            dr.InsPassFlag = True
                        Next

                        Dim intInsUpdate As Integer = ta.Update(dt)
                        'Dim drUpdateProd(intInsUpdate - 1) As DataSet1.T_Production_DATRow
                        'Dim index As Integer = 0
                        For Each drIns As DataSet1.T_Instruction_DATRow In dt

                            updateProduction(drIns, taProd)
                        Next
                        'Dim intProdUpdate As Integer = taProd.Update(dtUpdateProd)

                        logMSG = "Instruction changed from ModelCode = " & curModelYear & curSuffixCode
    ...
    Private Sub calCol()
        DataGridView1.Columns(0).Width = DataGridIdata.Columns(0).Width
        DataGridView1.Columns(1).Width = DataGridIdata.Columns(1).Width + DataGridIdata.Columns(2).Width
        DataGridView1.Columns(2).Width = DataGridIdata.Columns(3).Width + DataGridIdata.Columns(4).Width
        DataGridView1.Columns(3).Width = DataGridIdata.Columns(5).Width + DataGridIdata.Columns(6).Width + DataGridIdata.Columns(7).Width

        Dim tt As Integer = 0
        Dim i As Integer

        For i = 9 To DataGridIdata.Columns.Count - 1
            tt += DataGridIdata.Columns(i).Width
        Next

        DataGridView1.Columns(4).Width = tt

        DataGridView1.HorizontalScrollingOffset = DataGridIdata.HorizontalScrollingOffset
    End Sub

    'Sync datagridview scroll offset
    Private Sub DataGridIdata_Scroll(ByVal sender As System.Object, ByVal e As System.Windows.Forms.ScrollEventArgs) Handles DataGridIdata.Scroll
        DataGridView1.HorizontalScrollingOffset = DataGridIdata.HorizontalScrollingOffset
    End Sub

    Private Sub DataGridIdata_ColumnHeaderMouseClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles DataGridIdata.ColumnHeaderMouseClick
        'DataGridView1.HorizontalScrollingOffset = datagridPdata.HorizontalScrollingOffset
        calCol()
    End Sub

    Private Sub datagridPdata_MouseUp(ByVal sender As System.Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles DataGridIdata.MouseUp
        'DataGridView1.HorizontalScrollingOffset = datagridPdata.HorizontalScrollingOffset
        calCol()
    End Sub

    Private Sub DataGridIdata_CellMouseClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles DataGridIdata.CellMouseClick
        ...
        If e.Button = MouseButtons.Right Then
            ...
            If Me.DataGridIdata.CurrentRow.Cells(0).Value = Me.DataGridIdata.Item(0, e.RowIndex).Value Then
                ...
                Dim taInstruction As DataSet1TableAdapters.T_Instruction_DATTableAdapter = Me.m_taInst
                Dim tmpDtInst As DataSet1.T_Instruction_DATDataTable = DataGridIdata.DataSource
                Dim tmpDrInst As DataSet1.T_Instruction_DATRow
                Dim dt As DataSet1.T_Instruction_DATDataTable = Me.m_dtInst
    ...
    Private Sub DataGridIdata_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles DataGridIdata.KeyDown
        ...
        If e.Control And e.KeyCode = Keys.T Then
            ...
            Dim tmpDtInst As DataSet1.T_Instruction_DATDataTable = Me.m_dtTempInst
            Dim tmpDrInst As DataSet1.T_Instruction_DATRow
            Dim dt As DataSet1.T_Instruction_DATDataTable = DataGridIdata.DataSource
            Dim iCurrRow As Integer = DataGridIdata.CurrentRow.Index
